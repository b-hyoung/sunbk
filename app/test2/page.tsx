      ...
      <section className="bg-white py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div data-fade-up className="mb-12">
            <p className="text-navy-light text-xs font-bold tracking-[0.15em] uppercase mb-2">선박 종류</p>
            <h2 className="text-navy text-2xl font-bold">보유 선박 카테고리</h2>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {categories.map((cat) => (
              <Link
                ...
                href={cat.href}
                data-stagger
                className="group border border-border-light hover:border-navy-light p-8 transition-all hover:shadow-md text-center"
              >
                <div className="text-4xl mb-4">{cat.icon}</div>
                <h3 className="text-navy font-bold text-base group-hover:text-navy-light transition-colors">
                  {cat.label}
                </h3>
                <div className="mt-3 flex items-center justify-center gap-1 text-navy-light text-xs font-semibold md:opacity-0 md:group-hover:opacity-100 transition-opacity">
      ...
      <section className="bg-surface py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div data-fade-up className="flex items-end justify-between mb-12">
            <div>
              <p className="text-navy-light text-xs font-bold tracking-[0.15em] uppercase mb-2">Featured</p>
              <h2 className="text-navy text-2xl font-bold">추천 선박</h2>
            </div>
            <Link
              href="/test2/vessels"
              className="flex items-center gap-1 text-sm text-navy-light hover:text-navy-hover font-semibold transition-colors"
            >
          ...
          {featuredVessels.length > 0 ? (
            ...
            <div data-fade-in className="text-center py-20 text-navy/30">
