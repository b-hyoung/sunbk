...
const useCaseOrder: UseCase[] = ["survey", "construction"];
...
export default function VesselFilter({
  ...
  const useItems = [
    { label: "전체 용도", value: "" },
    ...useCaseOrder.map((uc) => ({
      label: `${USE_CASES[uc].icon} ${USE_CASES[uc].label}`,
      value: uc as string,
    })),
  ];
  ...
  const vesselTypeItems = [
    { label: "전체", value: "" },
    ...VESSEL_CATEGORIES.map((c) => ({ label: c, value: c as string })),
  ];
